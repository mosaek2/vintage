...
export default function QnaView() {
  ...
  useEffect(() => {
    axios
      .get(`${process.env.REACT_APP_API_URL}/board?uid=${uid}`, {
    ...
    axios
      .get(`${process.env.REACT_APP_API_URL}/comments?uid=${uid}`, {
        withCredentials: true,
      })
      .then((response) => {
      ...
      .then((response) => {
        if (response?.data?.uid === post?.member?.uid) {
          if (window.confirm("정말 삭제하시겠습니까?") === true) {
            axios
              .delete(
                `${process.env.REACT_APP_API_URL}/board/delete?uid=${uid}`,
                { withCredentials: true },
              )
              .then((response) => {
                console.log(response.data);
    ...
    <div id="QnaView">
      ...
      <Main>
        ...
        <div className="buttonContainer">
          <p className="info">
            {name}님이 {year}년 {month}월 {day}일 {hour}시 {minute}분에 작성한
            글입니다.
          </p>
