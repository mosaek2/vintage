...
export default function Item() {
  ...
  useEffect(() => {
    axios
      .get(`${process.env.REACT_APP_API_URL}/item?uid=${uid}`)
  ...
  return (
    <div id="Item">
      ...
      <Main>
        <div className="itemContainer">
          ...
          <div className="itemInfo">
            ...
            <div className="detailInfo">
              <div className="line">
                ...
                {item?.discountRate !== 1 ? (
                  <p className="discount">
                    -{100 - item?.discountRate * 100}% 할인
                  </p>
                ) : undefined}
                {item?.discountAmount !== 0 ? (
                  <p className="discount">
                    (-{item.discountAmount}원 추가 할인)
                  </p>
                ) : undefined}
                <p className="price">
                  {(item?.price * item?.discountRate - item?.discountAmount)
                    ?.toString()
                    ?.replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
                  원
                </p>
              ...
              {item?.sizeDetail !== null ? (
                <p>상세치수: {item?.sizeDetail}</p>
              ) : undefined}
              <p>
                제품상태:{" "}
                {item?.condition !== null
                  ? item?.condition
                  : "[중고] 사진 외에 특별한 오염이 없는 양호한 상태"}
              </p>
              <p>
                배송비:{" "}
                {item?.price * item?.discountRate - item?.discountAmount < 50000
                  ? "3000원"
                  : "무료"}
              </p>
            ...
            <div className="buttonContainer">
              ...
              <button className="white" onClick={handleClickQna}>
                문의하기
              </button>
        ...
        <img
          src="\images\items\guide1.jpg"
          className="guide guide1"
          alt="guide1"
        />
        <img
          src="\images\items\guide2.jpg"
          className="guide guide2"
          alt="guide2"
        />
        <img src={item?.detail} className="guide" alt="detail" />
        <img
          src="\images\items\guide3.jpg"
          className="guide guide3"
          alt="guide3"
        />
      </Main>
      <Footer />
