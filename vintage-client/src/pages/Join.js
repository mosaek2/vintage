...
export default function Join() {
    ...
    function formatPhoneNumber(value) {
        ...
        if (cleaned.length === 10) {
            ...
            if (match) {
                formattedNumber = [match[1], match[2], match[3]].filter(Boolean).join('-');
            }
        }
    ...
    function handleClickJoin() {
        ...
        if (birthYear < 1900 || birthYear > 2024 || birthMonth < 1 || birthMonth > 12 || birthDay < 1 || birthDay > 31) {
        ...
        <div id="Join">
            ...
            <Main>
                ...
                <div className="joinContainer">
                    ...
                    <div className="line">
                        <input style={{ width: 256 }} onChange={(e) => { setMail(e.target.value); setIsUniqueMailClick(false); }} />
                        <button className="buttonBox" onClick={handleClickMailCheck}>중복 확인</button>
                    </div>
                    ...
                    <input type="password" style={{ width: 256 }}
                        onChange={(e) => { setPassword(e.target.value); }}
                        onBlur={() => {
                            if (password !== "" && passwordCheck !== "") setIsValidPasswordCheckOut(true);
                            if (password !== "" && passwordCheck !== "" && password === passwordCheck) setIsValidPasswordCheck(true);
                    ...
                    <div className="line margin50">
                        ...
                        {isValidPasswordCheckOut ?
                            isValidPasswordCheck ?
                                <p className="blueWord">비밀번호가 일치합니다.</p> : <p className="redWord">비밀번호가 일치하지 않습니다.</p>
                            : undefined}
                    </div>
                    <input type="password" style={{ width: 256 }}
                    ...
                    <div className="line" style={{ gap: 0 }}>
                        <button className={gender === "male" ? "selectBoxSelect" : "selectBox"}
                            onClick={() => { setGender("male"); }}>남자</button>
                        <button className={gender === "female" ? "selectBoxSelect" : "selectBox"}
                    ...
                    <div className="line">
                        <div className="line" style={{ gap: 0 }}>
                            <button className={solarYn === 'y' ? "selectBoxSelect" : "selectBox"}
                                onClick={() => { setSolarYn('y'); }}>양력</button>
                            <button className={solarYn === 'n' ? "selectBoxSelect" : "selectBox"}
                                onClick={() => { setSolarYn('n'); }}>음력</button>
                        </div>
                        <div className="line" style={{ gap: 0 }}>
                            <input style={{ width: 45 }} onChange={(e) => { setBirthYear(e.target.value); }} />
                            <div className="birthBox">년</div>
                        </div>
                        <div className="line" style={{ gap: 0 }}>
                            <input style={{ width: 20 }} onChange={(e) => { setBirthMonth(e.target.value); }} />
                            <div className="birthBox">월</div>
                        </div>
                        <div className="line" style={{ gap: 0 }}>
                            <input style={{ width: 20 }} onChange={(e) => { setBirthDay(e.target.value); }} />
                    ...
                    <div className="line margin50">
                        ...
                        {isUniquePhoneClick ?
                            isUniquePhone ?
                                <p className="blueWord">사용 가능한 번호입니다.</p> : <p className="redWord">이미 사용 중인 번호입니다.</p>
                            : undefined}
                    </div>
                    <div className="line">
                        <input style={{ width: 148 }} onChange={handleChangePhone} value={phone} maxLength={13} />
                        <button className="buttonBox" onClick={handleClickPhoneCheck}>중복 확인</button>
                    </div>
                    ...
                    <input style={{ width: 148 }} onChange={handleChangePhoneAlt} value={phoneAlt} maxLength={13} readOnly />

                    <div className="line margin100">
                        <p className="joinKey">우편번호*</p>
                    </div>
                    <div className="line">
                        <input style={{ width: 70 }} onChange={(e) => { setZip(e.target.value); }}
                            name='zip' value={zip} readOnly />
                    ...
                    <input style={{ width: 600 }} onChange={(e) => { setAddress(e.target.value); }}
                    ...
                    <input style={{ width: 600 }} onChange={(e) => { setAddressDetail(e.target.value); }} />
                    ...
                    <div className="line" style={{ gap: 0 }}>
                        <button className={smsYn === 'y' ? "selectBoxSelect" : "selectBox"}
                            onClick={() => { setSmsYn('y'); }}>동의</button>
                        <button className={smsYn === 'n' ? "selectBoxSelect" : "selectBox"}
                            onClick={() => { setSmsYn('n'); }}>미동의</button>
                    </div>
                    ...
                    <div className="line" style={{ gap: 0 }}>
                        <button className={mailYn === 'y' ? "selectBoxSelect" : "selectBox"}
                            onClick={() => { setMailYn('y'); }}>동의</button>
                        <button className={mailYn === 'n' ? "selectBoxSelect" : "selectBox"}
                            onClick={() => { setMailYn('n'); }}>미동의</button>
                    </div>


                    <button className="joinRequest" onClick={handleClickJoin}>가입하기</button>
